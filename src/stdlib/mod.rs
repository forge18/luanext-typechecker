...
pub const BUILTINS: &str = include_str!("builtins.d.luax");

/// Lua 5.1 standard library
pub const LUA51: &str = include_str!("lua51.d.luax");

/// Lua 5.2 standard library
pub const LUA52: &str = include_str!("lua52.d.luax");

/// Lua 5.3 standard library
pub const LUA53: &str = include_str!("lua53.d.luax");

/// Lua 5.4 standard library
pub const LUA54: &str = include_str!("lua54.d.luax");

/// Lua 5.5 standard library
pub const LUA55: &str = include_str!("lua55.d.luax");

/// LuaJIT standard library
pub const LUAJIT: &str = include_str!("luajit.d.luax");

/// Reflection runtime module (available in all Lua versions)
pub const REFLECTION: &str = include_str!("reflection.d.luax");
...
pub fn get_all_stdlib(version: LuaVersion) -> Vec<(&'static str, &'static str)> {
    ...
    vec![
        ("builtins.d.luax", BUILTINS),
        (
            match effective_version {
                LuaVersion::Lua51 => "lua51.d.luax",
                LuaVersion::Lua52 => "lua52.d.luax",
                LuaVersion::Lua53 => "lua53.d.luax",
                LuaVersion::Lua54 => "lua54.d.luax",
                LuaVersion::Lua55 => "lua55.d.luax",
                LuaVersion::LuaJIT => "luajit.d.luax",
                LuaVersion::Auto => unreachable!("effective() should have resolved Auto"),
            },
            get_stdlib(version),
        ),
        ("reflection.d.luax", REFLECTION),
    ]
}
...
mod tests {
    ...
    #[test]
    fn test_lua55_embedded() {
        assert!(!LUA55.is_empty());
        // Lua 5.5 has the same stdlib API as 5.4
        assert!(LUA55.contains("declare function warn"));
        assert!(LUA55.contains("declare namespace coroutine"));
    }

    #[test]
    fn test_luajit_embedded() {
        assert!(!LUAJIT.is_empty());
        // LuaJIT extends Lua 5.1 with bit and jit namespaces
        assert!(LUAJIT.contains("declare namespace bit"));
        assert!(LUAJIT.contains("declare namespace jit"));
    }
    ...
    fn test_get_stdlib() {
        ...
        assert_eq!(get_stdlib(LuaVersion::Lua55), LUA55);
        assert_eq!(get_stdlib(LuaVersion::LuaJIT), LUAJIT);
    }
    ...
    fn test_get_all_stdlib() {
        ...
        assert_eq!(stdlib[0].0, "builtins.d.luax");
        assert_eq!(stdlib[1].0, "lua54.d.luax");
        assert_eq!(stdlib[2].0, "reflection.d.luax");
    }
}
